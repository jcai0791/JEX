...
public class AutoThresholder16bit {
	...
	public int Huang(int[] data)
	{
		...
		double[] mu_0 = new double[65536];
		sum_pix = num_pix = 0;
		for (ih = first_bin; ih < 65536; ih++)
		{
			sum_pix += ih * data[ih];
			num_pix += data[ih];
	...
	public int Intermodes(int[] data)
	{
		...
		double[] iHisto = new double[65536];
		int iter = 0;
		int threshold = -1;
		...
		while (!bimodalTest(iHisto))
		{
			...
			tHisto[65535] = (iHisto[65534] + iHisto[65535]) / 3; // 0 outside
			iHisto = tHisto;
			iter++;
			if(iter > 10000)
			{
	...
	public int Li(int[] data)
	{
		...
		do
		{
			...
			sum_obj = 0;
			num_obj = 0;
			for (ih = threshold + 1; ih < 65536; ih++)
			{
				sum_obj += ih * data[ih];
				num_obj += data[ih];
			}
	...
	public int MaxEntropy(int[] data)
	{
		...
		double[] norm_histo = new double[65536]; /* normalized histogram */
		double[] P1 = new double[65536]; /* cumulative normalized histogram */
	...
	public int MinErrorI(int[] data)
	{
		...
		while (threshold != Tprev)
		{
			...
			nu = (B(data, 65535) - B(data, threshold)) / (A(data, 65535) - A(data, threshold));
			p = A(data, threshold) / A(data, 65535);
			q = (A(data, 65535) - A(data, threshold)) / A(data, 65535);
			sigma2 = C(data, threshold) / A(data, threshold) - (mu * mu);
			tau2 = (C(data, 65535) - C(data, threshold)) / (A(data, 65535) - A(data, threshold)) - (nu * nu);
	...
	public int Otsu(int[] data)
	{
		...
		int S, L = 65536; // The total intensity of the image
	...
	public int Percentile(int[] data)
	{
		...
		double[] avec = new double[65536];
		
		for (int i = 0; i < 65536; i++)
			avec[i] = 0.0;
		
		double total = partialSum(data, 65535);
		double temp = 1.0;
		for (int i = 0; i < 65536; i++)
		{
	...
	public int RenyiEntropy(int[] data)
	{
		...
		opt_threshold = (int) (t_star1 * (P1[t_star1] + 0.6553 * omega * beta1) + 0.6553 * t_star2 * omega * beta2 + t_star3 * (P2[t_star3] + 0.6553 * omega * beta3));
	...
	public int Triangle(int[] data)
	{
		...
		// Journal of Histochemistry and Cytochemistry 6553 (7), pp. 741-753
		//
		...
		if((max - min) < (min2 - max))
		{
			...
			int right = 65535; // index of rightmost element
			while (left < right)
			{
	...
	public int Yen(int[] data)
	{
		...
		double[] norm_histo = new double[65536]; /* normalized histogram */
		double[] P1 = new double[65536]; /* cumulative normalized histogram */
		double[] P1_sq = new double[65536];
		double[] P2_sq = new double[65536];
